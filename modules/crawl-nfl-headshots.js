...
async function crawlImages() {
  const seasons = await Season.find({ sport: "nfl" });
...
async function generateThumbnails() {
  const directory = path.resolve(__dirname, "../assets/nfl-highres");
  fs.readdirSync(directory).forEach(async (file) => {
    const imgPath = path.resolve(__dirname, "../assets/nfl-highres", file);
