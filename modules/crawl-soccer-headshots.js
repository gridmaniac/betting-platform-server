...
async function crawlImages() {
  const seasons = await Season.find({ sport: "soccer" });
...
async function generateThumbnails() {
  const directory = path.resolve(__dirname, "../assets/soccer-highres");
  fs.readdirSync(directory).forEach(async (file) => {
    const imgPath = path.resolve(__dirname, "../assets/soccer-highres", file);
